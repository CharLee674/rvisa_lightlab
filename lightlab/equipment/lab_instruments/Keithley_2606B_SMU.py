...
class Remote_Keithley_2606B_SMU(RVISAInstrumentDriver):
    ...
    def __init__(
        ...
        **visa_kwargs
        ...
        self.host=url
        visa_kwargs["tempSess"] = visa_kwargs.pop("tempSess", True)
        RVISAInstrumentDriver.__init__(self, name=name, address=address, url=url, **visa_kwargs)
        self.reinstantiate_session(address, visa_kwargs["tempSess"], url)
    
    def _query(self, queryStr):
        return self._session_object.query(queryStr)
